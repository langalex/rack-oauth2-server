...
class ServerTest < Test::Unit::TestCase
  ...
  context "get_auth_request" do
    setup do
      @request = Server::AuthRequest.new(:database => DATABASE, :client_id => client.id, :scope => client.scope.join(" "),
  ...
  context "register" do
    context "no client ID" do
      ...
      should "create new client" do
        assert_equal 2, DATABASE.view(Server::Client.by_id(:reduce => true))
        assert_contains DATABASE.view(Server::Client.by_id).map(&:id), @client.id
      end
  ...
  context "access_grant" do
    ...
    context "with no scope" do
      ...
      should "pick client scope" do
        assert_equal %w{oauth-admin read write}, DATABASE.view(Server::AccessGrant.by_code(@code)).first.scope
      end
    end
  ...
  context "get_access_token" do
    ...
    context "with no scope" do
      ...
      should "pick client scope" do
        assert_equal %w{oauth-admin read write}, DATABASE.view(Server::AccessToken.by_token(@token)).first.scope
      end
    end
  end
