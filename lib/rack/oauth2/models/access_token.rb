  ...
  module OAuth2
    class AuthServer
      ...
      class AccessToken
        ...
        def generate_token
          self.token = AuthServer.secure_random
        end
        ...
        class << self
          ...
          def get_token_for(identity, client, scope)
            ...
            unless token = AuthServer.database.view(AccessToken.not_revoked_by_identiy_client_id_and_scope([identity, client.id, scope])).first
              token = AccessToken.new(:database =>  AuthServer.database, :client_id=>client.id, :identity=>identity, :scope=>scope)
              AuthServer.database.save token, false
            end
            token
          end
          ...
          #   end
          #   raw = AuthServer::AccessToken.collection.group("function (token) { return { ts: Math.floor(token.created_at / 86400) } }",
