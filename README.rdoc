*This fork uses CouchDB/Couch Potato instead of MongoDB*

= Rack::OAuth2::AuthServer
...
This release uses CouchDB. You're going to need
a running server and a +CouchPotato::Database+ object.
Because CouchDB is schema-less, there's no need to run migrations.

=== Step 2: Use The Server

For Rails 2.3/3.0, Rack::OAuth2::AuthServer automatically adds itself as middleware
...
+config/environment.rb+ (or one of the specific environment files). For example:
  ...
  Rails::Initializer.run do |config|
    ...
    config.after_initialize do
      config.oauth.database = CouchPotato.database
...
Rack::OAuth2::Sinatra into your application. For example:
  ...
  class MyApp < Sinatra::Base
    ...
    oauth.database = CouchPotato.database
...
With any other Rack server, you can +use Rack::OAuth2::AuthServer+ and pass your
own {Rack::OAuth2::AuthServer::Options} object.
...
Authorization requests go to +/oauth/authorize+. Rack::OAuth2::AuthServer
...
If the request passes validation, Rack::OAuth2::AuthServer sets the request header
...
it's a string. Rack::OAuth2::AuthServer intercepts this response and redirects the
...
403 (Forbidden). Rack::OAuth2::AuthServer will then redirect the user back to the
...
({Rack::OAuth2::AuthServer::Helper}) that cannot render anything, but can set the right
...
Rack::OAuth2::AuthServer intercepts all incoming requests and looks for an
...
If Rack::OAuth2::AuthServer finds a valid access token in the request, it sets the
...
Programatically, registering a new client is as simple as:
  ...
  > Rack::OAuth2::AuthServer.register(:display_name=>"UberClient",
...
file. For example, your +db/seed.rb+ may contain:
  ...
  Rack::OAuth2::AuthServer.register(id: oauth2["client_id"], secret: oauth2["client_secret"],
...
Rack::OAuth2::AuthServer also supports the so-called "two-legged" OAuth flow, which does
...
+config/environment.rb+:
  ...
  Rails::Initializer.run do |config|
    ...
    config.after_initialize do
      ...
      Rack::OAuth2::AuthServer::Admin.set :client_id, "4dca20453e4859cb000007"
      Rack::OAuth2::AuthServer::Admin.set :client_secret, "981fa734e110496fcf667cbf52fbaf03"
      Rack::OAuth2::AuthServer::Admin.set :scope, %w{read write}
    end
  end
...
split between Rack::OAuth2::AuthServer and your application, and the protocol the
...
The {Rack::OAuth2::AuthServer::Client} model represents the credentials of a client
...
authorization request is stored in the {Rack::OAuth2::AuthServer::AuthRequest}
...
An access grant ({Rack::OAuth2::AuthServer::AccessGrant}) is a nonce use to
...
An {Rack::OAuth2::AuthServer::AccessToken} is created by copying values from an
...
Rack::OAuth2::AuthServer was written to provide authorization/authentication for
...
Rack::OAuth2::AuthServer is available under the MIT license.
